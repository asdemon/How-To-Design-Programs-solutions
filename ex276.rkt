...
; title date LTracks -> LTracks
; It extracts from the latter the list of tracks that belong to the given album and have been played after the given date.
(check-expect (select-album-date-abstract "album" (create-date 1 2 3 4 5 6) cc)
              (list (create-track "name"
                "artist"
                "album"
                4
...
(define (select-album-date-abstract title date l)
  (filter (lambda (al) (= (compare (track-played al) date) 1))
          (filter (lambda (al) (string=? title (track-album al))) l)))

;  LTracks -> list of  LTracks.
; The function consumes an element of LTracks. It produce a list of LTracks, one per album.
(check-expect (select-albums-abstract cc)
              (list (list (create-track "name"
                "artist"
                "album"
                4
                ...
                (create-date 1 2 3 4 5 16)))
                    (list (create-track "one"
                                        ...
                                        7
                                        (create-date 1 2 3 4 5 50)))))
(define (select-albums-abstract l)
  (local
    (;acquire all album
     (define all-album
       (create-set (map track-album l))))
    ;---------
    (map (lambda (al) (filter (lambda (alf) (string=?  al (track-album alf))) l) ) all-album)))