...
(define (equal-consecutive s weight)
  (let ((s0 (stream-car s))
    ...
    (if (= (weight s0) (weight s1))
        (cons-stream (weight s0) (equal-consecutive (stream-cdr s) weight))
        (equal-consecutive (stream-cdr s) weight))))

(define (cube x)
  (* x x x))

;(display-stream-1 (weighted-pairs (integers-starting-from 1)
;                                                     (integers-starting-from 1)
;                                                     (lambda (x) (+ (cube (car x)) (cube (cadr x)))))
;10000)
(display-stream-1 (equal-consecutive (weighted-pairs (integers-starting-from 1)
                                                     (integers-starting-from 1)
                                                     (lambda (x) (+ (cube (car x)) (cube (cadr x)))))
                                     (lambda (x) (+ (cube (car x)) (cube (cadr x)))))
                  6)
