...
; Tetris  -> booleans
; deside whether to stop
(define (stop? s)
  (ormap posn-y-less-than-top? (tetris-landscape s)))
(define (posn-y-less-than-top? p)
  (posn-y-less-than? p 0))
(define (posn-y-less-than? p threshold)
  (<= (posn-y p) threshold))

(define (stop-picture s)
  (place-image/align (text "Game over" 16 "indigo")
                     0 0 "left" "top"
                     (tetris-render s)))
